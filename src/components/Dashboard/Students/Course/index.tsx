...
const StudentCourseComp = () => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/student/courses/all`,
        {
          params: { email, role },
        }
      );
  ...
  const handleCourseSubmit = async (courseIds: string[]) => {
    if (
      window.confirm("Are you sure you want to submit the selected courses?")
    ) {
      try {
        const response = await axios.patch(
          `${import.meta.env.VITE_API_URL}/student/courses/add`,
          {
            courseIds,
            email,
            role,
...
export default StudentCourseComp;
